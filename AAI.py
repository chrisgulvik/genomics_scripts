...
def parseArgs():
	...
	opt.add_argument('-f', '--fraction', type=float, metavar='FLOAT',
		default=30.0, help='minimum alignment length percentage [30.0]')
	opt.add_argument('-h', '--help', action='help',
		help='show this help message and exit')
	opt.add_argument('-i', '--identity', type=float, metavar='FLOAT',
		default=80.0, help='minimum percent identity [80.0]')
...
def main():
	...
	open(os.path.join(outpath, 'aai.loci.tab'), 'w') as loci:
		for line in dat:
			tot2 += 1
			l = line.split('\t')
			if float(l[2])>=opts.identity and int(l[3])>=opts.length \
	...
	# Write AAI data
	with open(os.path.join(outpath, 'aai.stats.tab'), 'w') as aai:
		aai.write('Sample(s)\tFiltered\tAAI\tStandard_Deviation\n')
		aai.write('{},{}\t{}/{}\t{}%\t{}%\n'.format(s2b, s1b, len(twoway),
			tot1+tot2, round(mean(twoway), 3), round(std(twoway,
			ddof=len(twoway)-1), 3)))
		aai.write('{}\t{}/{}\t{}%\t{}%\n'.format(s2b, filt1, tot1,
			round(mean(aai1), 3), round(std(aai1, ddof=filt1-1), 3)))
		aai.write('{}\t{}/{}\t{}%\t{}%\n'.format(s1b, filt2, tot2,
			round(mean(aai2), 3), round(std(aai2, ddof=filt2-1), 3)))
